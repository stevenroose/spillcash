use bitcoin::{
	absolute, opcodes, sighash, taproot, Address, Amount, CompressedPublicKey, Network, OutPoint, ScriptBuf, Sequence, Transaction, TxIn, TxOut, Witness
};
...
pub fn create_address(server: PublicKey, user: PublicKey) -> Address {
	...
	let taproot = taproot::TaprootBuilder::new()
		.add_leaf(0, delayed_sign(EXPIRY, user.into())).unwrap()
		.add_leaf(1, multisig(server, user)).unwrap()
...
pub fn update(
	...
	let mut tx = Transaction {
		...
		output: vec![
			...
			TxOut {
				value: user_amount,
				script_pubkey: ScriptBuf::new_p2pkh(&CompressedPublicKey(user.public_key()).pubkey_hash()),
			},
		],
	};
